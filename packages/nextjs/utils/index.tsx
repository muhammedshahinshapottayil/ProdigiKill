...
const PROPOSAL_RENEW_APPROVE_PENDING_COLUMNS = [
  {
    ...
    accessor: "reason",
    Cell: ({ row }: any) => {
      const { reason } = row.values;
      return reason.length < 100 ? (
        <p className={`text-gray-600 line-clamp-3`}>{reason}</p>
      ) : (
        <div>
          <p className={`text-gray-600 line-clamp-3`}>{reason}</p>
          <CustomModal
...
const PROPOSAL_SUBMITTED_APPROVE_PENDING_COLUMNS = [
  {
    Header: "Proof",
    accessor: "proof",
    Cell: ({ row }: any) => {
      const { proof } = row.values;
      return proof.length < 100 ? (
        <p className={`text-gray-600 line-clamp-3`}>{proof}</p>
      ) : (
        <div>
          <p className={`text-gray-600 line-clamp-3`}>{proof}</p>
          <CustomModal
  ...
  {
    Header: "Rating",
    accessor: "submitRating",
    Cell: ({ row }: any) => {
      const { submitRating } = row.values;
      return submitRating ? submitRating.length : 0;
    },
  },
  {
    Header: "Due Date",
